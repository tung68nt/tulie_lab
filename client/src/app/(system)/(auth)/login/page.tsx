...
export default function LoginPage() {
    ...
    const { isAuthenticated, isAdmin, refreshUser, isLoading: authLoading, login } = useAuth();
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // Redirect is handled by useEffect or here if needed immediately
            // Get user from local storage or context (context might not be updated in this scope immediately due to closure, but checking role)
            // We can just redirect to dashboard or admin based on localStorage if we really need to check role now,
            // or just redirect to dashboard and let middleware/guard handle it.
            // But since we just logged in, we can trust the response? user object is not returned by login() hook.
            // We can check local storage?
            ...
            addToast(error.message || 'Đăng nhập thất bại. Vui lòng kiểm tra lại email/mật khẩu.', 'error');
        } finally {
            setIsLoading(false);
        }
    };
