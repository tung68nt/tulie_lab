...
export class PrismaInstructorRepository implements IInstructorRepository {
    ...
    async findById(id: string): Promise<Instructor | null> {
        return prisma.instructor.findUnique({ where: { id }, include: { experiences: true } });
    }

    async findBySlug(slug: string): Promise<Instructor | null> {
        return prisma.instructor.findUnique({ where: { slug }, include: { experiences: true } });
    }

    async create(data: Prisma.InstructorCreateInput): Promise<Instructor> {
        return prisma.instructor.create({ data, include: { experiences: true } });
    }

    async update(id: string, data: Prisma.InstructorUpdateInput): Promise<Instructor> {
        return prisma.instructor.update({ where: { id }, data, include: { experiences: true } });
    }
